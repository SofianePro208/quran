document.addEventListener('DOMContentLoaded', () => {
    ...
    function setMode(mode) { if (mode !== 'reading') stopPlayback(); views.forEach(v => v.classList.remove('active')); document.getElementById(`${mode}-view`).classList.add('active'); modeButtons.forEach(b => b.classList.toggle('active', b.dataset.mode === mode)); if(mode === 'bookmarks') populateBookmarksView(); }
    ...
    function populateBookmarksView() { if (bookmarksList.innerHTML === null) return; if (bookmarks.length === 0) { bookmarksList.innerHTML = `<p class="ayah-placeholder" data-translate="noBookmarks">${TRANSLATIONS[currentLang].noBookmarks}</p>`; return; } bookmarksList.innerHTML = bookmarks.map(b => `<div class="bookmark-item"><div class="bookmark-header"><span class="bookmark-title">${b.surahName} (${b.surahNumber}:${b.ayahNumberInSurah})</span><button class="bookmark-remove-btn" data-id="${b.id}">${ICONS.remove}</button></div><div class="bookmark-content"><p class="ayah-text-arabic">${b.arabicText}</p><p class="ayah-text-english">“${b.englishText}”</p></div></div>`).join(''); }
    
    // --- API & Data Handling ---
    async function fetchAllData() {
        toggleLoader(true);
        const langCode = TRANSLATIONS[currentLang].langCode;
        try {
            const [recitersRes, suwarAudioRes, radiosRes, suwarTextRes] = await Promise.all([
                fetch(`${API_AUDIO_BASE}/reciters?language=${langCode}`),
                fetch(`${API_AUDIO_BASE}/suwar?language=${langCode}`),
                fetch(`${API_AUDIO_BASE}/radios?language=${langCode}`),
                fetch(`${API_TEXT_BASE}/meta`)
            ]);
            allReciters = (await recitersRes.json()).reciters;
            allReciters.sort((a, b) => a.name.localeCompare(b.name, currentLang === 'ar' ? 'ar-u-co-sort' : undefined));
            allSuwarAudio = (await suwarAudioRes.json()).suwar;
            allSuwarText = (await suwarTextRes.json()).data.surahs.references;
            allRadios = (await radiosRes.json()).radios;
            
            populateAllViews();
        } catch (error) { console.error('Failed to fetch data:', error); } finally { toggleLoader(false); }
    }
    ...
    function populateReciterSelects() { const savedReciterId = localStorage.getItem('quranReciterId') || '2'; const reciterId = savedReciterId; reciterSelects.forEach(select => { select.innerHTML = allReciters.map(r => `<option value="${r.id}" ${r.id == reciterId ? 'selected' : ''}>${r.name}</option>`).join(''); }); handleReciterChange({ target: { value: reciterId } }); }
    ...
    function handleReciterChange(event) {
        ...
        selectedReciter = allReciters.find(r => r.id == reciterId); if (!selectedReciter) return;
        localStorage.setItem('quranReciterId', reciterId);
        reciterSelects.forEach(s => s.value = reciterId);
        const riwayaOptions = selectedReciter.moshaf.map((m, i) => `<option value="${i}">${m.name}</option>`).join('');
        riwayaSelects.forEach(s => { s.innerHTML = riwayaOptions; s.disabled = false; });
        handleRiwayaChange({ target: { value: riwayaSelects[0].value }});
    }

    function handleRiwayaChange(event) {
        const moshafIndex = event.target.value;
        if(selectedReciter.moshaf[moshafIndex]) {
            selectedMoshaf = selectedReciter.moshaf[moshafIndex];
            riwayaSelects.forEach(s => s.value = moshafIndex);
            surahListListening.querySelectorAll('.item-card').forEach(item => item.classList.remove('locked'));
            audioPlayer.src = '';
        }
    }
    ...
    contactForm.addEventListener('submit', (e) => { e.preventDefault(); formSuccessMessage.classList.add('active'); contactForm.reset(); setTimeout(() => formSuccessMessage.classList.remove('active'), 5000); });

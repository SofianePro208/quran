document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateReciterSelects() { const savedReciterId = localStorage.getItem('quranReciterId') || '7'; /* Default to Abd El-Basit */ const reciterId = savedReciterId; reciterSelects.forEach(select => { select.innerHTML = allReciters.map(r => `<option value="${r.id}" ${r.id == reciterId ? 'selected' : ''}>${r.name}</option>`).join(''); }); handleReciterChange({ target: { value: reciterId } }); }
